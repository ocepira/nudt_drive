...
map_num_vec = 10                     # 100→10 (减少10倍)
map_fixed_ptsnum_per_gt_line = 5     # 20→5 (减少4倍)
map_fixed_ptsnum_per_pred_line = 5   # 20→5 (减少4倍)
...
_dim_ = 256                      # 256→64 (减少4倍)
_pos_dim_ = _dim_//2
_ffn_dim_ = _dim_*2
_num_levels_ = 1
bev_h_ = 25                          # 100→25 (减少4倍)
bev_w_ = 25                          # 100→25 (减少4倍)
queue_length = 1                     # 3→1 (减少3倍)
total_epochs = 1

model = dict(
    type='VAD',
    use_grid_mask=False,              # True→False (关闭grid_mask加速)
...
train_pipeline = [
    ...
    dict(type='RandomScaleImageMultiViewImage', scales=[0.1]),  # 0.4→0.1 (减少4倍)
...
test_pipeline = [
    ...
    dict(
        ...
        flip=False,
        transforms=[
            dict(type='RandomScaleImageMultiViewImage', scales=[0.1]),  # 0.4→0.1
            dict(type='PadMultiViewImage', size_divisor=32),
...
lr_config = dict(
    ...
    warmup='linear',
    warmup_iters=10,                    # 500→10 (减少50倍预热)
    warmup_ratio=1.0 / 3,
    min_lr_ratio=1e-3)

evaluation = dict(interval=1, pipeline=test_pipeline, metric='bbox', map_metric='chamfer')  # total_epochs→999 (几乎不评估)

runner = dict(type='EpochBasedRunner', max_epochs=total_epochs)

log_config = dict(
    interval=100,                      # 100→1000 (减少10倍日志频率)
    hooks=[
        dict(type='TextLoggerHook'),
        dict(type='TensorboardLoggerHook')
    ])
fp16 = dict(loss_scale=512.)          # 取消注释，启用混合精度
# find_unused_parameters = True
checkpoint_config = dict(interval=1, max_keep_ckpts=1)  # total_epochs→1 (只保留1个ckpt)
