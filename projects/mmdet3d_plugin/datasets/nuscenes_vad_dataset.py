...
class VADCustomNuScenesDataset(NuScenesDataset):
    ...
    def _evaluate_single(self,
        ...
        for metric in map_metrics:
            # print('-*'*10+f'use metric:{metric}'+'-*'*10)   屏蔽
            if metric == 'chamfer':
    ...
    def evaluate(self,
        ...
        for k, v in result_dict.items():
            # 确保值可以被序列化
            try:
                # 对于numpy标量，先转换为Python原生类型
                if isinstance(v, (np.integer, np.floating)):
                    value = float(v)
                else:
                    value = v
                sse_print(event='MotionPrediction', data={'metric': str(k), 'value': value})
            except Exception as e:
                print(f"Error serializing metric {k}: {e}")
                sse_print(event='MotionPrediction', data={'metric': str(k), 'value': 'serialization_error'})
            # print(f'{k}: {v}')
        ...
        for k in metric_dict:
            ...
            sse_print(event='planning', data={'metric': str(k), 'value': metric_dict[k]})
        ...
        # return results_dict
        # end = sse_print(event='end', data={'metric': 'end', 'value': 'end'})
