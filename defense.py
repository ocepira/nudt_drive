...
class GaussianDenoise:
    """
    Simple Gaussian denoising as baseline defense method.
    """
    ...
    def __call__(self, x, y=None):
        ...
        x_blurred = TF.gaussian_blur(x, self.kernel_size, self.sigma)
        
        out = x_blurred.round().clamp(0, 255).to(torch.uint8)
        if need_permute_back:
            out = out.permute(0, 2, 3, 1)
...
def create_defense(defense_method, **kwargs):
    ...
    elif defense_method.lower() == 'gaussian':
        return GaussianDenoise(
...
def main():
    ...
    # Gaussian 特有参数
    parser.add_argument('--kernel-size', type=int, default=5, help='高斯核大小')
    parser.add_argument('--sigma', type=float, default=1.0, help='高斯核标准差')
    ...
    try:
        ...
        elif args.defense_method.lower() == 'gaussian':
            defense = create_defense(
                args.defense_method,
